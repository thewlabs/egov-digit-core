...
public class MDMSService {
	...
	public Map<String, Map<String, JSONArray>> search(MdmsCriteriaReq mdmsCriteriaReq) {
		final Map<String, Map<String, JSONArray>> tenantMasterMap = new HashMap<>();
		/*
		...
		schemaCodes.entrySet().forEach(entry -> {
			...
			tenantMasterNewMap = applyFilterToData(tenantMasterNewMap,
					mdmsCriteriaReq.getMdmsCriteria().getSchemaCodeFilterMap());

			Map<String, Map<String, JSONArray>> finalTenantMasterMap = new HashMap<>();
			for (Map.Entry<String, Map<String, JSONArray>> map : tenantMasterNewMap.entrySet()) {
				Map<String, JSONArray> newHashMap = new HashMap<>();
				newHashMap.putAll(map.getValue());
				finalTenantMasterMap.put(mdmsCriteriaReq.getMdmsCriteria().getTenantId(), newHashMap);
			}

			Map<String, JSONArray> newHashMap = new HashMap<>();

			if (!tenantMasterNewMap.isEmpty()
					&& !tenantMasterNewMap.get(mdmsCriteriaReq.getMdmsCriteria().getTenantId()).isEmpty()) {
				newHashMap.putAll(tenantMasterNewMap.get(mdmsCriteriaReq.getMdmsCriteria().getTenantId()));
			}

			if (tenantMasterMap.isEmpty()
					|| tenantMasterMap.get(mdmsCriteriaReq.getMdmsCriteria().getTenantId()).isEmpty()) {
				tenantMasterMap.put(mdmsCriteriaReq.getMdmsCriteria().getTenantId(), newHashMap);
			} else {
				tenantMasterMap.get(mdmsCriteriaReq.getMdmsCriteria().getTenantId()).putAll(newHashMap);
			}
		});
